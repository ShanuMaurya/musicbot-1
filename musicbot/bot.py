...
class MusicBot(discord.Client):
    ...
    async def cmd_ayy(self, channel):
        """
        Usage:
            {command_prefix}ayy
        Ayy command to test latency
        """
        await self.safe_send_message(channel, "lmao") 
 ...
 in the settings file.", delete_after=30)
    ...
    async def cmd_help(self, command=None):
        ...
        if command:
            ...
            helpmsg += "```"
            helpmsg += "Here is the list of all my commands :point_up::skin-tone-1:"
    ...
    async def cmd_lyric(self, channel, player):
        ...
        for item in songsResults:
            try:
                ...
                return Response(":thumbsdown::skin-tone-1:")
            except IndexError:      
    ...
    async def cmd_shuffle(self, channel, player):
        ...
        return Response(":ok_hand::skin-tone-1:", delete_after=15)
    ...
    async def cmd_clear(self, player, author):
        ...
        return Response('Playlist successfully cleared :put_litter_in_its_place:', delete_after=20)
    ...
    async def cmd_volume(self, message, player, new_volume=None):
        ...
        if 0 < new_volume <= 100:
            ...
            return Response('Successfully changed volume from %d to %d' % (old_volume, new_volume), reply=True, delete_after=20)
    ...
    async def cmd_queue(self, channel, player):
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                lines.append("I'm Currently Playing: **%s** added by **%s** %s\n" % (
    ...
    async def cmd_clean(self, message, channel, server, author, search_range=50):
        ...
        if self.user.bot:
            if channel.permissions_for(server.me).manage_messages:
                ...
                return Response('I cleaned {} message{}.'.format(len(deleted), 's' * bool(deleted)), delete_after=15)
        ...
        return Response('I cleaned {} message{}.'.format(deleted, 's' * bool(deleted)), delete_after=15)
    ...
    async def cmd_listids(self, server, author, leftover_args, cat='all'):
        ...
        return Response("Check your mail :mailbox_with_mail:", delete_after=20)
    ...
    async def cmd_repeat(self, player):
        ...
        if player.is_repeatNone:
            return Response(":play_pause: Repeat mode: Disabled", delete_after=20)
        if player.is_repeatAll:
            return Response(":repeat: Repeat mode: Always", delete_after=20)
        if player.is_repeatSingle:
            return Response(":repeat_one: Repeat mode: Once", delete_after=20)
    ...
    async def cmd_setname(self, leftover_args, name):
        ...
        return Response("Name successfully changed! :ok_hand::skin-tone-1:", delete_after=20)
    ...
    async def cmd_setnick(self, server, channel, leftover_args, nick):
        ...
        return Response("Nickname successfully changed! :ok_hand::skin-tone-1:", delete_after=20)
    ...
    async def cmd_setavatar(self, message, url=None):
        ...
        return Response(":ok_hand::skin-tone-1:", delete_after=20)

    async def cmd_disconnect(self, server):
        await self.disconnect_voice_client(server)
        return Response("Successfully disconnected from ðŸŽµ Music ðŸŽµ", delete_after=20)

    async def cmd_restart(self, channel):
        await self.safe_send_message(channel, ":wave::skin-tone-1: bye bye http://pa1.narvii.com/5654/a8d93adf332f69c87f8af0ebfcf7d37d3031eca3_hq.gif")
        await self.disconnect_all_voice_clients()
